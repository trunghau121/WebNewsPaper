...
class Preg_matchBM {
    ...
    public function get_data($arr) {
        $subject = Preg_matchBM::getURL($arr->host);
        ...
        foreach ($matches[1] as $key => $info) {
            ...
            $tmp['content'] = Preg_matchBM::get_content($url, $pre_content, $remove);
    ...
    public function get_content($url, $pre, $remove) {
        ...
        $subject = Preg_matchBM::getURL($url);
        $pattern = $pre;
        preg_match_all($pattern, $subject, $matches);
        if (!empty(trim($remove))) {
            $matches[0][0] = Preg_matchBM::RemoveTag($remove, $matches[0][0]);
        }
        return $matches[0][0];
    }
