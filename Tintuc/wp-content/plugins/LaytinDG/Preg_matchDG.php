...
class Preg_matchDG {
    ...
    public function get_data($arr) {
        $subject = Preg_matchDG::getURL($arr->host);
        ...
        foreach ($matches[1] as $key => $info) {
            ...
            $tmp['content'] = Preg_matchDG::get_content($url, $pre_content, $remove);
    ...
    public function get_content($url, $pre, $remove) {
        ...
        $subject = Preg_matchDG::getURL($url);
        $pattern = $pre;
        preg_match_all($pattern, $subject, $matches);
        if (!empty(trim($remove))) {
            $matches[0][0]=  Preg_matchDG::RemoveTag($remove,$matches[0][0]);
        }
        return $matches[0][0];
    }
