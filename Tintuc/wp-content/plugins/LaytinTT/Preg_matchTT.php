...
class Preg_matchTT {
    ...
    public function get_data($arr) {
        $subject = Preg_matchTT::getURL($arr->host);
        ...
        foreach ($matches[1] as $key => $info) {
            ...
            $tmp['content'] = Preg_matchTT::get_content($url, $pre_content, $remove);
    ...
    public function get_content($url, $pre, $remove) {
        ...
        $subject = Preg_matchTT::getURL($url);
        $pattern = $pre;
        preg_match_all($pattern, $subject, $matches);
        if (!empty(trim($remove))) {
            $matches[0][0]=  Preg_matchTT::RemoveTag($remove,$matches[0][0]);
        }
        return $matches[0][0];
    }
